...
(* ::Text::GrayLevel[0.5]:: *)
(* Autogenerated PublicPacletServer loader file *)


Temp`PackageScope`PublicPacletServerLoading`Private`$DependencyLoad=
  TrueQ@Temp`PackageScope`PublicPacletServerLoading`Private`$DependencyLoad;
If[Temp`PackageScope`PublicPacletServerLoading`Private`$DependencyLoad,
  Unprotect["`PublicPacletServer`PackageScope`Private`Constants`$TopLevelLoad"];
  Evaluate[Symbol["`PublicPacletServer`PackageScope`Private`Constants`$TopLevelLoad"]]=False,
  Unprotect["PublicPacletServer`PackageScope`Private`Constants`$TopLevelLoad"];
  Evaluate[Symbol["PublicPacletServer`PackageScope`Private`Constants`$TopLevelLoad"]]=
    MemberQ[$ContextPath, "Global`"]
  ];


If[Temp`PackageScope`PublicPacletServerLoading`Private`$DependencyLoad,
  Evaluate[Symbol["`PublicPacletServer`PackageScope`Private`$RootContext"]]=$Context;
  BeginPackage["`PublicPacletServer`"],
  Evaluate[Symbol["PublicPacletServer`PackageScope`Private`$RootContext"]]="";
  BeginPackage["PublicPacletServer`"]
  ]
...
PublicPacletServer["Directory"]:=
  $PackageDirectory;
$PackageDirectory=
  DirectoryName@$InputFileName;


PublicPacletServer["Name"]:=
  $PackageName;
$PackageName=
  "PublicPacletServer";
...
PublicPacletServer["LoadingParameters"]:=$PackageLoadSpecs
$PackageLoadSpecs=
  Merge[
    {
      With[
...
PublicPacletServer["Mode"]:=$PackageLoadingMode
$PackageLoadingMode=
...
PublicPacletServer["Dependencies"]:=$PackageDependencies;
$PackageDependencies=
...
PublicPacletServer["DependencyBase"]:=$PackageDependencyBase;
...
PublicPacletServer["FEScopedSymbols"]:=$PackageFEHiddenSymbols;
$PackageFEHiddenSymbols={};
PublicPacletServer["PackageScopedSymbols"]:=$PackageScopedSymbols;
...
PublicPacletServer["AllowRescoping"]:=$AllowPackageRescoping;
$AllowPackageRescoping=
  Replace[
    Lookup[$PackageLoadSpecs, "AllowRescoping"],
    Except[True|False]->($PackageLoadingMode==="Primary")(*$TopLevelLoad*)
    ];
PublicPacletServer["AllowRecoloring"]:=$AllowPackageRecoloring;
$AllowPackageRecoloring=
  Replace[
    Lookup[$PackageLoadSpecs, "AllowRecoloring"],
    Except[True|False]->($PackageLoadingMode==="Primary")(*$TopLevelLoad*)
    ];
PublicPacletServer["AllowAutocompletions"]:=$AllowPackageAutocompletions;
...
PublicPacletServer["FileContexts"]:=$PackageFileContexts;
...
PublicPacletServer["DeclaredPackages"]:=$DeclaredPackages;
...
PublicPacletServer["LoadedPackages"]:=$LoadedPackages;
...
PackageFileContext[f_String?DirectoryQ]:=
  With[{s=PackageFileContextPath[f]},
    StringRiffle[Append[""]@Prepend[s,PublicPacletServer],"`"]
    ];
...
PublicPacletServer["Load", args___]:=
  PackageAppLoad[args]
  ...
  With[{
    newcont=
      If[StringStartsQ[context, "`"],
        "PublicPacletServer"<>context<>scope<>"`",
        context<>scope<>"`"
        ],
  ...
  Block[{retcode, site, path, file, tmp},
    ...
    site=
      ...
      If[retcode<300,
        ...
        DeleteFile[tmp];
        file,
        Message[PublicPacletServer::nodep, dep, "Package"];
        DeleteFile[tmp];
        $Failed
        ]
      ];
    ...
    Monitor[
      MapThread[
        Check[
          ...
          Message[PublicPacletServer::nodep, #2, "Paclet"];
          $Failed
          ]&,
        {
          pacs,
      ...
      Monitor[
        MapThread[
          Check[
            ...
            Message[PublicPacletServer::nodup, #2, "Paclet"];
            $Failed
            ]&,
          {
            StringDelete[deps, "`"],
...
PackageCatchMessage[
  ...
  Replace[
    Thread[Map[Hold[MessageName[PublicPacletServer, #]]&, msg], Hold],
    {
      {}:>
        Check[expr, 
...
PackageAttachMainAutocomplete[]:=
  PackageAddAutocompletions[PublicPacletServer, 
    Table[
      Replace[{}->None]@
        Cases[
          DownValues[PublicPacletServer],
          Nest[
            Insert[#, _, {1, 1, 1}]&,
            (HoldPattern[Verbatim[HoldPattern]][
              PublicPacletServer[s_String, ___]
              ]:>_),
            n-1
            ]:>s,
          Infinity
...
PackagePrepPackageSymbol[]:=
  Switch[$AllowPackageSymbolDefinitions,
    None,
      If[Length@OwnValues[PublicPacletServer]==0,
        Remove[PublicPacletServer],
        DownValues[PublicPacletServer]={}
        ],
    False,
      If[Length@OwnValues[PublicPacletServer]==0,
...
If[Temp`PackageScope`PublicPacletServerLoading`Private`$DependencyLoad,
  If[
    (Clear@`PublicPacletServer`PackageScope`Private`$loadAbort;!#)&@
      `PublicPacletServer`PackageScope`Private`$loadAbort,
    Unprotect[`PublicPacletServer`PackageScope`Private`$LoadCompleted];
    `PublicPacletServer`PackageScope`Private`$LoadCompleted=True;
    `PublicPacletServer`PackageScope`Private`PackagePostProcessContextPathReassign[]
    ],
  If[
    (Clear@PublicPacletServer`PackageScope`Private`$loadAbort;!#)&@
      PublicPacletServer`PackageScope`Private`$loadAbort,
...
Remove["Temp`PackageScope`PublicPacletServerLoading`Private`*"]